...
const totalVolume = function (solids) {
  ...
  solids.forEach((element) => {
    if (element.type === 'sphere') {
      totalArray.push(sphereVolume(element.radius))
    } else if (element.type === 'cone') {
      totalArray.push(coneVolume(element.radius, element.height))
    } else if (element.type == 'prism') {
      totalArray.push(prismVolume(element.height, element.width, element.depth))
    }
  })
